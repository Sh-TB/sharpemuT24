...
public static partial class KernelMemoryCompatExports
{
    ...
    public static int KernelOpenUnderscore(CpuContext ctx)
    {
        ...
        {
            // TASK-008: trace failed open
            try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath ?? "<unreadable>", "open", false); }
            catch { /* diagnostics must never crash emulator */ }
            return (int)OrbisGen2Result.ORBIS_GEN2_ERROR_MEMORY_FAULT;
        }

        // TASK-008: We will trace once we know whether the open succeeded.
        // (The hook needs the success/failure boolean.)
        ...
        try
        {
            if (Bink2MovieBridge.ShouldSkipGuestMovie(hostPath))
            {
                ...
                try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, "bink-skip", false); }
                catch { /* diagnostics must never crash emulator */ }
            ...
            if (IsMutatingOpen(flags) && IsReadOnlyGuestMutationPath(guestPath))
            {
                ...
                try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, "readonly-denied", false); }
                catch { /* diagnostics must never crash emulator */ }
            ...
            if (wantsDirectory || Directory.Exists(hostPath))
            {
                if (!Directory.Exists(hostPath))
                {
                    ...
                    try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, "directory", false); }
                    catch { /* diagnostics must never crash emulator */ }
                    return (int)OrbisGen2Result.ORBIS_GEN2_ERROR_NOT_FOUND;
                }
                ...
                try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, "directory", true); }
                catch { /* diagnostics must never crash emulator */ }
            ...
            // TASK-008: trace successful open
            try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, mode.ToString(), true); }
            catch { /* diagnostics must never crash emulator */ }
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            ...
            try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileOpen(guestPath, "open", false); }
            catch { /* diagnostics must never crash emulator */ }
    ...
    public static int KernelStat(CpuContext ctx)
    {
        ...
        if (statCacheKey is not null && IsNegativeStatCached(statCacheKey))
        {
            ...
            // TASK-008: trace negative-cached stat
            try { SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileStat(guestPath, false); }
            catch { /* diagnostics must never crash emulator */ }
            return (int)OrbisGen2Result.ORBIS_GEN2_ERROR_NOT_FOUND;
        }
    ...
    public static int KernelReadUnderscore(CpuContext ctx)
    {
        ...
        // TASK-008: time the read for diagnostic tracing
        var readStartTicks = System.Diagnostics.Stopwatch.GetTimestamp();
        var buffer = GC.AllocateUninitializedArray<byte>(requested);
        var read = stream.Read(buffer, 0, requested);
        var readElapsedMs = (System.Diagnostics.Stopwatch.GetTimestamp() - readStartTicks) * 1000.0
        ...
        // TASK-008: trace successful read
        try
        {
            SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileRead(
                stream.Name,
                (ulong)Math.Max(0, positionBefore),
                (ulong)read,
                readElapsedMs);
        }
        catch { /* diagnostics must never crash emulator */ }
    ...
    public static int KernelWriteUnderscore(CpuContext ctx)
    {
        ...
        if (fd == 1 || fd == 2)
        {
            ...
            // TASK-008: trace write to stdout/stderr (use fd as path label)
            try
            {
                SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileWrite(
                    fd == 1 ? "<stdout>" : "<stderr>",
                    0,
                    (ulong)requested,
                    0.0);
        ...
        long positionBefore;
        try { positionBefore = stream.Position; }
        catch (IOException) { positionBefore = -1; }
        
        var writeStartTicks = System.Diagnostics.Stopwatch.GetTimestamp();
        stream.Write(payload, 0, requested);
        stream.Flush();
        var writeElapsedMs = (System.Diagnostics.Stopwatch.GetTimestamp() - writeStartTicks) * 1000.0
            / System.Diagnostics.Stopwatch.Frequency;

        // TASK-008: trace write to file
        try
        {
            SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.NotifyFileWrite(
                stream.Name,

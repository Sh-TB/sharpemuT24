...
public static class LibcStdioExports
{
    ...
    /// <summary>Creates a lowercase conversion table (tobyte) for _Getptolower.</summary>
    public static unsafe nint EnsureCtypeLowerTable()
    {
        lock (_ctypeLowerTableGate)
        {
            if (_ctypeLowerTableBase != 0) return _ctypeLowerTableBase;
            var storage = Marshal.AllocHGlobal(CtypeTableEntryCount * sizeof(ushort));
            var entries = new Span<ushort>((void*)storage, CtypeTableEntryCount);
            for (var i = 0; i < CtypeTableEntryCount; i++)
            {
    ...
    /// <summary>Creates an uppercase conversion table (tobyte) for _Getptoupper.</summary>
    public static unsafe nint EnsureCtypeUpperTable()
    {
        lock (_ctypeUpperTableGate)
        {
            if (_ctypeUpperTableBase != 0) return _ctypeUpperTableBase;
            var storage = Marshal.AllocHGlobal(CtypeTableEntryCount * sizeof(ushort));
            var entries = new Span<ushort>((void*)storage, CtypeTableEntryCount);
            for (var i = 0; i < CtypeTableEntryCount; i++)
            {

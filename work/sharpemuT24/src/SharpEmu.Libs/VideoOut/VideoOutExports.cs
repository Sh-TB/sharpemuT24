...
public static class VideoOutExports
{
    ...
    private const int MaxFrameDumps = 100;
    ...
    private sealed class VideoOutPortState
    {
        ...
        /// <summary>
        /// Indicates this port is using headless/fake display (no real GPU).
        /// </summary>
        public bool IsHeadlessPort { get; init; }
    }
    ...
    public static int VideoOutOpen(CpuContext ctx)
    {
        ...
        // [DIAGNOSTICS] GPU: VideoOut opened
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
                diag.Publish(DiagnosticEvent.VideoOutOpen(index));
        }
        catch { /* diagnostics must not crash */ }

        // ============================================================
        // VIDEOOUT MANAGER INTEGRATION
        // Ensure backend is selected before opening display
        // ============================================================
        
        // Initialize VideoOutManager if not already done
        // This is the SINGLE entry point for GPU backend selection
        if (!VideoOutManager.IsHeadlessMode && !VideoOutManager.BackendReason.Contains("GPU detected"))
        {
            VideoOutManager.Initialize();
        }
        
        // If in headless mode, use fake display handle
        if (VideoOutManager.IsHeadlessMode)
        {
            Console.Error.WriteLine("[VIDEOOUT][INTEGRATION] sceVideoOutOpen → Headless mode, using fake display");
            var fakeHandle = VideoOutManager.AllocateDisplayHandle();
            
            // Still register in ports for compatibility
            lock (_stateGate)
            {
                ...
                _ports[fakeHandle] = new VideoOutPortState
                {
                    Handle = fakeHandle,
                    OpenTimestamp = openedAt,
                    LastVblankTimestamp = openedAt,
                    IsHeadlessPort = true  // Mark as headless port
                };
            }
        ...
        // Existing Vulkan path continues below...
        ...
        bool submitGpuImage)
    {
        ...
        // Route flips from headless ports to VideoOutManager
        // ============================================================
        if (port.IsHeadlessPort && VideoOutManager.IsHeadlessMode)
        {
            // Get buffer info if available
            ulong address = 0;
            uint width = port.OutputWidth;
            uint height = port.OutputHeight;
            uint pitch = width; // Default pitch
            
            if (bufferIndex >= 0 && TryGetDisplayBufferInfo(handle, bufferIndex, out var headlessDisplayBuffer))
            {
                address = headlessDisplayBuffer.Address;
                width = headlessDisplayBuffer.Width;
                height = headlessDisplayBuffer.Height;
                pitch = headlessDisplayBuffer.PitchInPixel;
            }
            
            // ROOT CAUSE FIX: Read actual framebuffer pixel data from guest memory
            // instead of letting HeadlessVideoPresenter generate a synthetic test pattern.
            // The game writes pixels to this address via CPU (Construct 2 engine)
            // or AGC command buffer. We read them here and pass to the presenter.
            byte[]? frameData = null;
            if (address != 0 && width > 0 && height > 0)
            {
                try
                {
                    var pixelSize = (int)(width * height * 4);  // RGBA8 = 4 bytes/pixel
                    frameData = new byte[pixelSize];
                    if (!ctx.Memory.TryRead(address, frameData))
                    {
                        // Guest memory read failed — fallback to test pattern
                        Console.Error.WriteLine($"[VIDEOOUT][FB] TryRead FAILED for addr=0x{address:X16} size={pixelSize}");
                        frameData = null;
                    }
                    else
                    {
                        ...
                        int nonZero = 0;
                        for (int i = 0; i < Math.Min(frameData.Length, 1000); i++)
                        {
                            if (frameData[i] != 0) nonZero++;
                        }
                        Console.Error.WriteLine($"[VIDEOOUT][FB] TryRead OK for addr=0x{address:X16} size={pixelSize} nonZero(first1000)={nonZero}");
                    }
                }
                catch { frameData = null; }
            }
            
            // Route to VideoOutManager for headless processing
            VideoOutManager.Flip(handle, bufferIndex, address, width, height, pitch, frameData);
            
            Console.Error.WriteLine($"[VIDEOOUT][INTEGRATION] Flip → Headless: handle={handle} buf={bufferIndex} frame=#{VideoOutManager.FlipCount}");
            
            // [DIAGNOSTICS] GPU: Flip event
            try
            {
                var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
                if (diag?.IsActive == true)
                    diag.Publish(DiagnosticEvent.GpuFlip(bufferIndex));
            }
            catch { /* diagnostics must not crash */ }
            
            // Still trigger flip events for game compatibility
            TriggerFlipEventsForHeadless(port, bufferIndex);
    ...
    /// <summary>
    /// Triggers flip events for headless mode (simplified version).
    /// Games expect flip events to be signaled even without real GPU.
    /// </summary>
    private static void TriggerFlipEventsForHeadless(VideoOutPortState port, int bufferIndex)
    {
        // In headless mode, we still need to signal flip events to keep the game running
        // This is a simplified version that doesn't require GPU synchronization
        try
        {
            var eventHint = SceVideoOutInternalEventFlip;
            
            foreach (var flipEvent in port.FlipEvents)
            {
                _ = KernelEventQueueCompatExports.TriggerDisplayEvent(
                    flipEvent.Equeue,
                    SceVideoOutInternalEventFlip,
        ...
        {
            Console.Error.WriteLine($"[VIDEOOUT][HEADLESS] Warning: Failed to trigger flip events: {ex.Message}");
        }
    }

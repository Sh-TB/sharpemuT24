...
public sealed class SharpEmuRuntime : ISharpEmuRuntime
{
    ...
    public OrbisGen2Result Run(string ebootPath)
    {
        ...
        // [DIAGNOSTICS] Boot stage: SelfImage loaded
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
                diag.Publish(SharpEmu.Logging.DiagnosticEvent.BootStage("SelfImage", $"{image.TitleId} @ 0x{image.EntryPoint:X16}"));
        }
        ...
        // [DIAGNOSTICS] Boot stage: Initializers completed
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
                diag.Publish(SharpEmu.Logging.DiagnosticEvent.BootStage("Initializers", "All module initializers executed successfully"));
        }
        catch { /* diagnostics must not crash */ }
        ...
        // [DIAGNOSTICS] Boot stage: EntryPoint reached
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
                diag.Publish(SharpEmu.Logging.DiagnosticEvent.BootStage("EntryPoint", $"0x{image.EntryPoint:X16} gen={generation}"));
        }
        catch { /* diagnostics must not crash */ }

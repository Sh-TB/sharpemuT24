...
internal static partial class Program
{
    ...
    private static int RunEmulator(string[] args, bool isMitigatedChild)
        {
            ...
            // ============================================
            // CRASH EXPORT PIPELINE - Initialize Diagnostics
            // ============================================
            var gameName = Path.GetFileNameWithoutExtension(ebootPath) ?? "unknown";
            using var diagnostics = new DiagnosticEngine(gameName);
            
            // [DIAGNOSTICS] Initialize Intelligence Engine (replaces old static scaffolding)
            var profile = ResolveDiagnosticProfile();
            // BUG-FIX: CreateSession takes (gameId, sessionDir, profile). 
            // We pass a session dir under the diagnostics base path.
            var sessionDir = Path.Combine(diagnostics.DiagnosticsBasePath, "session_" + DateTime.UtcNow.ToString("yyyyMMdd_HHmmss"));
            Directory.CreateDirectory(sessionDir);
            using var intelEngine = DebugIntelligenceEngine.CreateSession(gameName, sessionDir, profile);
            
            // [DIAGNOSTICS v3.0] Register the engine as the active event bus via the Contracts adapter.
            // This is the new architecture: Core/Libs/HLE call DiagnosticAdapter.Notify*(),
            // which routes to the registered IDiagnosticEventBus (our engine).
            SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.RegisterBus(intelEngine);
            
            // PHASE 2-A/B/C/D FIX: Activate diagnostics in DirectExecutionBackend so that
            // RecordInstructionExecuted() actually forwards to the bus (was being skipped
            // because _diagnosticsActive was always false).
            SharpEmu.Core.Cpu.Native.DirectExecutionBackend.ActivateDiagnostics();
            
            // PHASE 2-B FIX: Register SignalSafeCrashWriter as the crash source so that
            // CaptureCrashContextFromSignal() -> NotifyCrash() -> QueueCrash() actually
            // reaches the writer and generates crash_*.json files.
            SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.RegisterCrashSource(
                SharpEmu.Diagnostics.SignalSafeCrashWriter.Instance);
            
            Console.Error.WriteLine($"[DIAG] Intelligence Engine activated: profile={profile}");
            Console.Error.WriteLine($"[DIAG] Subsystems: CPU Trace, Crash Writer, Memory Map, GPU State, Thread Debug, Syscall, File IO, HLE Quality");
            Console.Error.WriteLine($"[DIAGNOSTICS] Engine initialized: {diagnostics.SessionDirectory}");
            Console.Error.WriteLine($"[DIAGNOSTICS] Live session: {Path.Combine(diagnostics.LiveDirectory, "session.json")}");
            ...
            if (!File.Exists(ebootPath))
            {
                ...
                diagnostics.RecordError("FileNotFound", ebootPath);
                diagnostics.GeneratePackage();
                return 2;
            }
            ...
            try
            {
                cancelHandler = (_, eventArgs) =>
                {
                    ...
                    diagnostics.RecordError("UserInterrupt", "Ctrl+C pressed");
                };
            ...
            catch (Exception ex)
            {
                ...
                diagnostics.RecordError(ex.GetType().Name, ex.Message, ex.StackTrace);
                return 3;
            }
            finally
            {
                // ============================================
                // CRASH EXPORT PIPELINE - Always flush!
                // ============================================
                Console.Error.WriteLine("[DIAGNOSTICS] Finally block - flushing diagnostics...");
                ...
                // ALWAYS flush - even on crash, exit, etc.
                try
                {
                    diagnostics.Flush();
                    diagnostics.GeneratePackage();
                    Console.Error.WriteLine($"[DIAGNOSTICS] Package generated at: {diagnostics.SessionDirectory}");
                }
                catch (Exception diagEx)
                {
                    Console.Error.WriteLine($"[DIAGNOSTICS] Flush failed: {diagEx.Message}");
                }
                
                // [DIAG] Generate Intelligence Engine report (root cause analysis)
                try
                {
                    if (intelEngine?.IsActive == true)
                    {
                        intelEngine.GeneratePackage();
                        var (confidence, summary, _) = intelEngine.AnalyzeRootCause();
                        Console.Error.WriteLine($"[DIAG] Intelligence Report: {intelEngine.SessionDirectory}");
                        Console.Error.WriteLine($"[DIAG] Confidence: {confidence}%");
                        Console.Error.WriteLine($"[DIAG] Imports tracked: {intelEngine.TotalImports:N0}");
                        Console.Error.WriteLine($"[DIAG] Events captured: {intelEngine.TotalEvents:N0}");
                        
                        // Print maintainer-friendly output
                        var maintainerPath = Path.Combine(intelEngine.SessionDirectory, "maintainer.txt");
                        if (File.Exists(maintainerPath))
                        {
                            Console.Error.WriteLine("--- MAINTAINER REPORT ---");
                            Console.Error.WriteLine(File.ReadAllText(maintainerPath));
                        }
                    }
                }
                catch (Exception intelEx)
                {
                    Console.Error.WriteLine($"[DIAG] Intel engine report failed: {intelEx.Message}");
                }
                
                // [DIAG v3.0] Generate FULL diagnostic package from engine
                try
                {
                    if (intelEngine?.IsActive == true)
                    {
                        intelEngine.GeneratePackage();
                        Console.Error.WriteLine($"[DIAG] Full package: {intelEngine.SessionDirectory}");
                    }
                }
                catch (Exception coordEx)
                {
                    Console.Error.WriteLine($"[DIAG] Full package failed: {coordEx.Message}");
                }
            }
    ...
    /// <summary>
    /// Resolves the diagnostic profile from environment variables.
    /// SHARPEMU_DIAG_PROFILE=compat|deep|dev|normal (default: compat)
    /// </summary>
    private static DiagnosticProfile ResolveDiagnosticProfile()
    {
        var env = Environment.GetEnvironmentVariable("SHARPEMU_DIAG_PROFILE");
        if (string.IsNullOrWhiteSpace(env))
            return DiagnosticProfile.Compatibility; // Default for game testing
        
        return env.ToLowerInvariant() switch
        {
            "normal" => DiagnosticProfile.Normal,
            "compat" or "compatibility" => DiagnosticProfile.Compatibility,
            "deep" or "debug" => DiagnosticProfile.DeepDebug,
            "dev" or "developer" => DiagnosticProfile.Developer,
            _ => DiagnosticProfile.Compatibility
        };
    }
}

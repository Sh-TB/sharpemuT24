...
public sealed unsafe class PhysicalVirtualMemory : IVirtualMemory, IGuestMemoryAllocator, IGuestAddressSpace, IDisposable
{
    ...
    public bool TryAllocateGuestMemory(ulong size, ulong alignment, out ulong address)
    {
        ...
        lock (_guestAllocationGate)
        {
            ...
            // [DIAGNOSTICS] Track guest memory allocation
            try
            {
                var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
                if (diag?.IsActive == true)
                    diag.Publish(DiagnosticEvent.MemoryAlloc(address, size));
            }
    ...
    public bool TryFreeGuestMemory(ulong address)
    {
        lock (_guestAllocationGate)
        {
            ...
            // [DIAGNOSTICS] Track guest memory free
            try
            {
                var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
                if (diag?.IsActive == true)
                    diag.Publish(DiagnosticEvent.MemoryFree(address, allocation.Size));
            }
            catch { /* diagnostics must not crash */ }

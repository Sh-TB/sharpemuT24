...
public sealed unsafe partial class DirectExecutionBackend
{
        ...
        // BUG-FIX: must be const for use in switch expressions.
        // On macOS, SIGBUS is signal 10; on Linux it's signal 7.
        // We pick the Linux value as default since this stub runs on Linux for our test.
        private const int PosixSigBus = 7;
        ...
        private static bool TryHandlePosixFault(int signal, nint siginfo, nint ucontext)
        {
                ...
                {
                        // [DIAGNOSTICS] Unrecovered crash - record before process terminates
                        try
                        {
                                ...
                                if (diag?.IsActive == true)
                                {
                                        ...
                                        var sigName = signal switch
                                        {
                                                PosixSigSegv => "SIGSEGV",
                                                PosixSigBus => "SIGBUS",
                                                PosixSigIll => "SIGILL",
                                                PosixSigTrap => "SIGTRAP",
                                                PosixSigAbort => "SIGABRT",
                                                _ => $"signal({signal})"
                                        };
                                        diag.Publish(DiagnosticEvent.Crash(sigName, 
                                                $"Unrecovered {sigName} at RIP=0x{rip:X16} fault=0x{faultAddr:X16}",
                                                faultAddr));
                                        
                                        // BUG-A7 FIX (newly discovered): Actually call CaptureCrashContextFromSignal
                                        // so the SignalSafeCrashWriter receives the crash data and can generate
                                        // the full crash package (registers.txt, crash.json, cpu_trace.txt, etc.).
                                        // Previously this code path only published a DiagnosticEvent but never
                                        // invoked the two-phase crash writer — so crash packages were never
                                        // generated on Linux/macOS.
                                        // Note: contextRecord here is a Win64-format CONTEXT that we filled
                                        // from the POSIX ucontext_t above. CaptureCrashContextFromSignal
                                        // detects the platform and uses the right offsets.
                                        CaptureCrashContextFromSignal(sigName, faultAddr, contextRecord);
                                        
                                        // Force immediate flush before process dies
                                        diag.Flush();
                                }
                        }
                        catch { /* must not throw from signal handler */ }

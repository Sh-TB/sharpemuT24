...
public sealed partial class DirectExecutionBackend
{
        ...
        private unsafe int VectoredHandler(void* exceptionInfo)
        {
                ...
                try
                {
                        ...
                        // EXP-046: Handle INT3 from crash_path lookup tracer.
                        if (exceptionCode == 2147483651u && Exp046TryHandleCrashPathInt3(contextRecord, rip))
                        {
                                return -1;
                        }
                        ...
                        // EXP-098: Handle INT3 from registration path tracers
                        if (exceptionCode == 2147483651u && Exp098TryHandleWorkingInitInt3(contextRecord, rip))
                        {
                                return -1;
                        }
                        if (exceptionCode == 2147483651u && Exp098TryHandleDeadRegInt3(contextRecord, rip))
                        {
                                return -1;
                        }
                        ...
                        // EXP-101: Handle INT3 from PLT stub return value tracers
                        if (exceptionCode == 2147483651u && Exp101TryHandlePLTCallInt3(contextRecord, rip))
                        {
                                return -1;
                        }
                        if (exceptionCode == 2147483651u && Exp101TryHandlePLTRetInt3(contextRecord, rip))
                        {
                                return -1;
                        }
                        // EXP-102: Handle INT3 from callback storage tracer
                        if (exceptionCode == 2147483651u && Exp102TryHandleXchgInt3(contextRecord, rip))
                        {
                                return -1;
                        }

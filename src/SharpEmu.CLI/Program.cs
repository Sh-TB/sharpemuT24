    ...
    private static int RunEmulator(string[] args, bool isMitigatedChild)
        {
            ...
            // ============================================
            // ============================================
            // DIAGNOSTICS v3 — Plugin-based EventBus architecture
            // ============================================
            ...
            var sessionDir = Path.Combine(diagDir, "session_" + DateTime.UtcNow.ToString("yyyyMMdd_HHmmss"));
            using var diagManager = new SharpEmu.Diagnostics.DiagnosticManager(gameName, sessionDir);

            // CaptureCrashContextFromSignal() -> NotifyCrash() -> QueueCrash() actually
            // reaches the writer and generates crash_*.json files.
// Removed old diagnostics initialization
...
//             Console.Error.WriteLine($"[DIAG] Intelligence Engine activated: profile={profile}");
            ...
            Console.Error.WriteLine($"[DIAGNOSTICS] Engine initialized: {sessionDir}");
//             Console.Error.WriteLine($"[DIAGNOSTICS] Live session: {Path.Combine(diagManager.LiveDirectory, "session.json")}");
            ...
            if (!File.Exists(ebootPath))
            {
                ...
                // diagManager.RecordError("FileNotFound", ebootPath);
                // diagManager.GeneratePackage();
                return 2;
            }
            ...
            try
            {
                cancelHandler = (_, eventArgs) =>
                {
                    ...
                    // diagManager.RecordError("UserInterrupt", "Ctrl+C pressed");
                };
            ...
            catch (Exception ex)
            {
                ...
                // diagManager.RecordError(ex.GetType().Name, ex.Message, ex.StackTrace);
                return 3;
            }
            finally
            {
                ...
                // ============================================
                Console.Error.WriteLine("[DIAGNOSTICS] Finally block - flushing diagManager...");
                ...
                try
                {
                    // diagManager.Flush();
                    // diagManager.GeneratePackage();
                    Console.Error.WriteLine($"[DIAGNOSTICS] Package generated at: {sessionDir}");
                }
                ...
                // [DIAG v3] DiagnosticManager.Dispose() will flush all plugin reports
                try
                {
                    if (diagManager?.IsActive == true)
                    {
                        Console.Error.WriteLine($"[DIAG] Reports written to: {sessionDir}");
                    }
                }
                catch { }

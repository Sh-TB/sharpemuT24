...
public static class KernelPthreadCompatExports
{
    ...
    {
        // [DIAGNOSTICS] Thread state: Locking mutex
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
            {
                var tid = KernelPthreadState.GetCurrentThreadHandle();
                diag.Publish(DiagnosticEvent.ThreadState((int)tid, tryOnly ? "MutexTryLock" : "MutexLock", $"mutex=0x{mutexAddress:X16}"));
            }
        }
    ...
    {
        // [DIAGNOSTICS] Thread state: Waiting on condition variable
        try
        {
            var diag = SharpEmu.Diagnostics.Contracts.DiagnosticAdapter.Bus;
            if (diag?.IsActive == true)
            {
                var tid = KernelPthreadState.GetCurrentThreadHandle();
                diag.Publish(SharpEmu.Logging.DiagnosticEvent.ThreadState((int)tid, "CondWait", $"cond=0x{condAddress:X16}"));
            }
        }
        catch { /* diagnostics must not crash */ }
